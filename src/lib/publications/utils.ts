...
const BIBTEX_EXCLUDED_FIELD_PATTERN =
  /^(?:selected|equal|image|authormarkers|previewvideo)/i
...
function applyEqualMarkers(
  ...
  if (typeof pub.authormarkers === "string") {
    for (const pair of pub.authormarkers.split(",")) {
      const [position, symbols] = pair.split(":")
      ...
      if (
        ...
        result[index] += `<sup>${symbols}</sup>`
      }
    }
    return {
      authors: result,
      note: "* Equal contribution. Other author markers follow the original papers",
    }
  }
...
export function getPublicationData(
  ...
  return {
    ...
    previewVideo: publication.previewvideo as string | undefined,
